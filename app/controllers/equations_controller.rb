class EquationsController < ApplicationController
  ...
  def create
    ...
    uri = '/'
    @url = host + uri
    public_key = File.read(Rails.root + 'config/keys/id_rsa.pub')
      .split(' ')[1]
    type = eq_params[:type]
    a = eq_params[:a]
    b = eq_params[:b]
    ...
    str = ''
    File.readlines(Rails.root + 'config/keys/id_rsa').each do |line|
      str << line if line[0] != '-'
    end
    private_key = str.gsub(/\n/, '')
    computed_signature = Base64.encode64(OpenSSL::HMAC
      .digest(OpenSSL::Digest.new('sha256'), uri + body, private_key)).strip
